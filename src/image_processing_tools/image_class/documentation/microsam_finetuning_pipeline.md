...
- **Not a default in AIS-only mode**: when `train_instance_segmentation_only=True` and no sampler is passed, `default_sam_dataset` does **not** auto-apply this sampler (`training.py`: `if sampler is None and not train_instance_segmentation_only`). It is *not* disabled, though — a sampler you pass explicitly is still used in full (the multi-GPU and overfit scripts pass `MinInstanceSampler(2, min_size=25)` and it runs normally).

> **⚠️ This sampler should be removed from the AIS training scripts.** It was added on 2026-06-08 while debugging NaN training loss and all-black prediction rows, under the hypothesis that empty patches produce an all-zero distance transform that divides by zero in `normalize_im` and poisons the weights. **That diagnosis was wrong.** The real cause was the GTX 1660 Ti's defective fp16 path (§10.1), which NaNs on the first iteration regardless of patch content and is fixed by bf16 / fp32 / a Tensor-Core GPU (T4). The sampler did not fix anything and was never reverted, so it remained active in `candida_multigpu_ais.py`, `candida_finetune_all.py`, and `candida_overfit_debug.py`, including the real T4 runs.
>
> Keeping it is not harmless. `default_sam_dataset` omits this sampler for AIS-only training **on purpose** — the AIS decoder is meant to see background-only patches so it learns to predict empty foreground there, i.e. to recognize background as background (the code comment: *"if we train only the automatic instance segmentation decoder, then this sampler is not required and we do not set a default sampler"*). Forcing `MinInstanceSampler(2, min_size=25)` removed every background-only patch from training, biasing the decoder toward predicting foreground everywhere. This is a plausible contributor to the background regions misclassified as cells seen in the evaluation. **Action:** delete the `sampler=` argument from the three AIS training scripts to restore micro-sam's reference recipe (no sampler for AIS-only), then re-run to confirm fewer background false positives.
