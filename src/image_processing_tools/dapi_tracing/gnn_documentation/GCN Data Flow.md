...
- **Angle Differences (`node1_angle_diff`, `node2_angle_diff`, `min_diff_angle`, `relative_angle`):** The angular deviation between the major axis of the cell and the proposed connection path. `node1_angle_diff` is the **source** node's deviation and `node2_angle_diff` the **target**'s, per directed edge — see [Data preprocessing](#Data%20preprocessing). `min_diff_angle` and `relative_angle` are order-independent.
...
- **Per-direction angle features:** `T.ToUndirected()` copies each `edge_attr` row verbatim onto the reverse edge. That is correct for the four order-independent features (`average_intensity`, `length`, `min_diff_angle`, `relative_angle` — each is a mean, a distance, a `min`, or a folded difference), but **not** for `node1_angle_diff` and `node2_angle_diff`, which are measured against the row's own source and target. `extract_graph` emits each pair once with `source < target`, so the copied row described the *target* and the *source* respectively on every reverse edge — backwards, and silently: both values stay in `[0, 1]` and nothing raises. `create_pyg_data` now swaps the two columns wherever `source > target`, so both mean what they are named on every direction. **Why it matters:** message passing computes `A → B` and `B → A` separately, and before the fix both received the same angle attribution, keyed to whichever endpoint happened to hold the lower node index rather than to the direction of travel. The swap keys on the `source < target` invariant, which is asserted rather than assumed. Runs built before this change are not comparable to runs after it.
