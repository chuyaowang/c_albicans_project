...
- **How:** For each node, a fixed-size axis-aligned box (default 150×150 px, configurable) is built around its centroid. For each edge, a box spanning source and target centroids is built, padded by a fractional margin (default 15%) with a minimum pixel floor (default 20 px) so very short edges still get a reasonable receptive field. Boxes are fed through torchvision's `roi_align` with `spatial_scale = 1 / pixels_per_feature`, producing a fixed `(256, roi, roi)` patch per node and per edge (default `roi=7`).
- **Why:** Different edges span wildly different pixel distances, and different nuclei have different sizes. A fixed bounding box per node + a data-adaptive box per edge means the CNN downstream always receives the same-shape tensor regardless of the underlying geometry. The edge box is deliberately loose (fractional overshoot) because the biologically relevant signal — the hyphal tube connecting the two nuclei — often extends slightly outside the tight centroid-to-centroid bounding rectangle.
