...
def _node_boxes(centroids, box_size):
    """Centroids (N, 2) in (y, x) -> boxes (N, 4) in (x1, y1, x2, y2)."""
    half = box_size / 2.0
    y = centroids[:, 0]
    x = centroids[:, 1]
...
def _edge_boxes(centroids, edge_index, margin_frac, margin_floor):
    """Edge boxes: bbox of the two endpoint centroids, padded by fractional margin + floor."""
...
class Model(torch.nn.Module):
    ...
    def _extract_visual(self, data):
        ...
        edge_xyxy = _edge_boxes(centroids, edge_index, self.edge_box_margin_frac, self.edge_box_margin_floor)
