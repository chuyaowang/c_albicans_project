...
def create_pyg_data(
    ...
    for i, (edge_index, nuclei_df, path_df, edge_label) in enumerate(zip(edge_indices, nuclei_features_list, path_features_list, edge_labels_list)):
        ...
        # right for the symmetric features (average_intensity, length, min_diff_angle,
        # relative_angle -- all order-independent by construction), but node1_angle_diff and
        # node2_angle_diff are measured against the row's own source and target.
        # `extract_graph` emits each pair once with source < target, so on a reverse edge the
        # copy describes the target and the source respectively: backwards, and silently,
        # since every value stays in range. Swap them there, so both columns mean what they
        # are named on every direction.
        ...
        if {"node1_angle_diff", "node2_angle_diff"} <= set(edge_cols):
            ...
            # Keys on source > target, which identifies the copy only because the producer
