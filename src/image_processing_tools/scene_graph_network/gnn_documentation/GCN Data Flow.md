...
- **Per-direction angle features:** `T.ToUndirected()` copies each `edge_attr` row **verbatim** onto the reverse edge. That is right for every feature that is symmetric by construction — a mean, a distance, a `min`, a folded difference (nuclei: 4 of 6; fragments: 8 of 10, the extra `contact` / `area_ratio` / `collinearity` / `continuity` all being symmetric too) — but **not** for `node1_angle_diff` and `node2_angle_diff`, which are measured against the row's own source and target. The producers emit each pair once with `source < target`, so the copied row described the *target* and the *source* respectively on every reverse edge: backwards, and **silently**, since both values stay in `[0, 1]` and nothing raises. `create_pyg_data` now swaps the two columns wherever `source > target` (`gnn_data.py:215-229`), so both mean what they are named on every direction.

    **Why it matters:** message passing computes `A → B` and `B → A` separately, and before the fix both received the same angle attribution — keyed to whichever endpoint happened to hold the lower node index rather than to the direction of travel. The swap keys on the `source < target` invariant, which is **asserted rather than assumed** (`gnn_data.py:220`), because the fix is silently wrong if a producer ever stops canonicalising.

    ⚠️ **Runs built before this change are not comparable to runs after it** — two of the edge features changed meaning. This includes every nuclei-pipeline result predating it.
