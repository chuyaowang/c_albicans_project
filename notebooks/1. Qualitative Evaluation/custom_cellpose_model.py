...
from cellpose.dynamics import follow_flows, get_masks_torch, remove_bad_flow_masks
...
class CellposeModelCustomMap():
    ...
    def eval(self, x, batch_size=8, resample=True, channels=None, channel_axis=None,
             ...
             flow3D_smooth=0, stitch_threshold=0.0, flow_field_threshold=30.0,
    ...
    def _compute_masks(self, shape, dP, cellprob, flow_threshold=0.4, cellprob_threshold=0.0,
                       ...
                       flow_field_threshold=30.0): # Add new arg
        ...
        if do_3D:
            masks = dynamics.resize_and_compute_masks(
                ...
                flow_field_threshold=flow_field_threshold, # Pass new arg
            ...
            for i in iterator:
                ...
                outputs = resize_and_compute_masks(
                    dP[:, i], cellprob[i],
...
def resize_and_compute_masks(dP, cellprob, niter=200, cellprob_threshold=0.0,
    ...
    if resize is not None:
        models_logger.warning("Resizing is deprecated in v4.0.1+")
...
def compute_masks(dP, cellprob, p=None, niter=200, cellprob_threshold=0.0,
    ...
    # 1. Generate RGB flow field from dP components
    flow_field_rgb = plot.dx_to_circ(dP)

    # 2. Create the combined mask using your custom function
    combined_mask = combine_thresholded_maps(
        cell_prob_map=cellprob,
        flow_field=flow_field_rgb,
        prob_threshold=cellprob_threshold,
        flow_threshold=flow_field_threshold
    )
    
    if combined_mask.sum() > 0:
        inds = np.nonzero(combined_mask)
        if len(inds[0]) == 0:
        ...
        p_final = follow_flows(dP * combined_mask / 5.,
